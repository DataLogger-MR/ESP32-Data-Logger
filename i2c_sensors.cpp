...
#include "ads1115_scheduler.h"   // for i2cMutex
#include <map>
#include <memory>
...
// ================ HELPER FUNCTIONS WITH MUTEX PROTECTION ================

static bool i2cWriteWithTimeout(uint8_t addr, const uint8_t* data, size_t len, int timeoutMs) {
    if (xSemaphoreTake(i2cMutex, pdMS_TO_TICKS(timeoutMs + 10)) != pdTRUE) return false;
...
static bool isMCP9600Present() {
    ...
    if (Wire.endTransmission(false) == 0) {
        if (Wire.requestFrom((uint8_t)MCP9600_ADDR, (uint8_t)2) == 2) {
            uint8_t highByte = Wire.read();
            Wire.read();
            present = (highByte == 0x40);
        }
    }
    xSemaphoreGive(i2cMutex);
    return present;
...
static float readMCP9600Temperature() {
    float temp = NAN;
    if (xSemaphoreTake(i2cMutex, pdMS_TO_TICKS(10)) != pdTRUE) return NAN;
    Wire.beginTransmission(MCP9600_ADDR);
    Wire.write(0x00);
    if (Wire.endTransmission(false) == 0) {
        if (Wire.requestFrom((uint8_t)MCP9600_ADDR, (uint8_t)2) == 2) {
...
// ================ INITIALIZATION (protected via helpers) ================
...
void scanI2CBus() {
    ...
    for (address = 1; address < 127; address++) {
        ...
        unsigned long start = millis();
        if (xSemaphoreTake(i2cMutex, pdMS_TO_TICKS(10)) != pdTRUE) continue;
...
// ================ UPDATE FUNCTIONS WITH MUTEX PROTECTION ================
...
// ================ NEW: Read a single pair of thermocouples ================
void readThermocouplePair(int pairIndex) {
    if (!useMultiplexer) return;
    if (pairIndex < 0 || pairIndex >= 6) return;

    int firstIdx = pairIndex * 2;
    int secondIdx = firstIdx + 1;
    if (secondIdx >= HARDCODED_MCP9600_COUNT) return;

    unsigned long now = millis();

    // Read first thermocouple
    HardcodedMCP9600& tc1 = hardcodedMCP9600s[firstIdx];
    if (tc1.valid) {
        if (selectMCP9600Path(tc1)) {
            delay(5);
            float temp = readMCP9600Temperature();
            float ambient = NAN;
            if (!isnan(temp)) {
            ...
            if (!isnan(temp) && temp > -50 && temp < 500) {
                sensorData.thermocouples[firstIdx].temperature = temp;
                sensorData.thermocouples[firstIdx].ambientTemp = ambient;
                sensorData.thermocouples[firstIdx].fault = 0;
                sensorData.thermocouples[firstIdx].valid = true;
                sensorData.thermocouples[firstIdx].lastUpdate = now;
            } else {
                sensorData.thermocouples[firstIdx].valid = false;
                sensorData.thermocouples[firstIdx].fault = 1;
            }
            resetI2CPath();
        } else {
            sensorData.thermocouples[firstIdx].valid = false;
    ...
    // Read second thermocouple
    HardcodedMCP9600& tc2 = hardcodedMCP9600s[secondIdx];
    if (tc2.valid) {
        if (selectMCP9600Path(tc2)) {
            delay(5);
            float temp = readMCP9600Temperature();
            float ambient = NAN;
            if (!isnan(temp)) {
            ...
            if (!isnan(temp) && temp > -50 && temp < 500) {
                sensorData.thermocouples[secondIdx].temperature = temp;
                sensorData.thermocouples[secondIdx].ambientTemp = ambient;
                sensorData.thermocouples[secondIdx].fault = 0;
                sensorData.thermocouples[secondIdx].valid = true;
                sensorData.thermocouples[secondIdx].lastUpdate = now;
            } else {
                sensorData.thermocouples[secondIdx].valid = false;
                sensorData.thermocouples[secondIdx].fault = 1;
            }
            resetI2CPath();
        } else {
            sensorData.thermocouples[secondIdx].valid = false;
...
void updateADS1115() {
    // Not used with the scheduler
}

// ================ populateI2CValues – updated to set sensorData.lastScanTime ================
void populateI2CValues() {
    if (xSemaphoreTake(i2cValuesMutex, pdMS_TO_TICKS(10)) == pdTRUE) {
        unsigned long now = millis();
        sensorData.lastScanTime = now;   // protected
        ...
        for (const auto& dev : i2cConfig.devices) {
            if (dev.type == "MCP23017") {
                ...
                const ADS1115Reading* reading = getADS1115Reading(addr);
                if (!reading) continue;
                for (const auto& sig : dev.signals) {
                    if (!sig.enabled) continue;
                    if (sig.channel >= 0 && sig.channel < ADS1115_CHANNELS) {
                        if (reading->valid[sig.channel]) {
                            double phys = reading->voltage[sig.channel] * sig.factor + sig.offset;
                            i2cValues[sig.name] = phys;
                        }
                    }
                }
...
void updateI2CSensors() {
    ...
    // These are now called from the scheduler slots, so we don't call them here.
    // But we keep populateI2CValues to refresh the i2cValues map if needed.
