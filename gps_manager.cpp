...
// ================ GLOBALS ================
TinyGPSPlus gps;
SoftwareSerial gpsSerial(GPS_RX_PIN, GPS_TX_PIN);  // GPIO16, GPIO17
...
void initGPS() {
    gpsSerial.begin(gpsBaudRate);  // Just baud rate, pins are set in constructor
    delay(100);
...
void updateGPS() {
    ...
    // Take mutex before updating shared structures
    if (xSemaphoreTake(gpsMutex, pdMS_TO_TICKS(10)) == pdTRUE) {
        ...
        if (gpsData.altitude_valid) gpsData.altitude = gps.altitude.meters();
        ...
        if (gpsData.hdop_valid) gpsData.hdop = gps.hdop.hdop();
        if (gpsData.satellites_valid) gpsData.satellites = gps.satellites.value();
